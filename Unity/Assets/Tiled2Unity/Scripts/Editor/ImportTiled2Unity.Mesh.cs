        ...
        private void AddGameObjectsTo(GameObject parent, XElement xml, bool isParentTrigger, string objPath, IList<ICustomTiledImporter> customImporters)
        {
            foreach (XElement goXml in xml.Elements("GameObject"))
            {
                ...
                if (!String.IsNullOrEmpty(copyFrom))
                {
                    ...
                    {
                        OrderinLayerSpriteSorting sort = renderer.gameObject.AddComponent<OrderinLayerSpriteSorting>();
                        XAttribute attr = goXml.Attribute("sortingOrder");
                        int sortOrder = ImportUtils.GetAttributeAsInt(goXml, "sortingOrder", 0);
                        if (sortOrder > 0)
                        {
                            float offset = renderer.bounds.extents.y * 40f;
                            sort.offset = -50 - Mathf.RoundToInt(offset);
                        }
                        else
                        {
                            sort.offset = sortOrder;
                        }
                    }
                    else
                    {
